...
add_action( 'after_switch_theme', 'wordpress_gwinnett_switch_theme'   );
add_action( 'load-customize.php', 'wordpress_gwinnett_load_customize' );
add_action( 'template_redirect',  'wordpress_gwinnett_preview'        );
 ...
 * @return string
 */
function wordpress_gwinnett_compat_message() {

	if ( version_compare( $GLOBALS['wp_version'], '4.9.6', '<' ) ) {

		return sprintf(
			// Translators: 1 is the required WordPress version and 2 is the user's current version.
			__( 'WordPress Gwinnett requires at least WordPress version %1$s. You are running version %2$s. Please upgrade and try again.' ),
			'4.9.6',
			$GLOBALS['wp_version']
		);
		...
		return sprintf(
			...
			__( 'WordPress Gwinnett requires at least PHP version %1$s. You are running version %2$s. Please upgrade and try again.' ),
			'5.6',
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function wordpress_gwinnett_switch_theme( $old_name ) {

	switch_theme( $old_name ? $old_name : WP_DEFAULT_THEME );

	unset( $_GET['activated'] );

	add_action( 'admin_notices', 'wordpress_gwinnett_upgrade_notice' );
}
 ...
 * @return void
 */
function wordpress_gwinnett_upgrade_notice() {

	printf( '<div class="error"><p>%s</p></div>', esc_html( wordpress_gwinnett_compat_message() ) );
}
 ...
 * @return void
 */
function wordpress_gwinnett_load_customize() {

	wp_die( esc_html( wordpress_gwinnett_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function wordpress_gwinnett_preview() {

	if ( isset( $_GET['preview'] ) ) { // WPCS: CSRF ok.
		wp_die( esc_html( wordpress_gwinnett_compat_message() ) );
	}
}
